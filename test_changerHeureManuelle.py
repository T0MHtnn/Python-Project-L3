...
class tKInterApp(tk.Tk):
    ...
    def update_app_time(self):
        # Obtenir l'heure actuelle avec le décalage UTC
        current_time = self.get_datetime_with_utc()
        now = current_time.strftime("%H:%M:%S")

        # Obtenir la région à partir du décalage UTC
        region = self.get_region_from_utc_offset(self.time_offset)

        # Mettre à jour le label de l'heure dans l'application
        if hasattr(self, 'app_time_label'):
            self.app_time_label.config(text=f"Fuseau Horaire: {now} (UTC{self.time_offset:+}) - {region}")
        else:
            print("Warning: app_time_label is not initialized.")

        # Mettre à jour les horloges dans les frames
        if hasattr(self, 'frames'):
            for frame in self.frames.values():
                if isinstance(frame, (Horloge, DHorloge)):
                    frame.update_clock()
        else:
            print("Warning: frames is not initialized.")
    ...
    def open_timezone_window(self):
        ...
        self.create_time_adjustment_section(bg_color, fg_color, entry_bg_color)

    def create_timezone_section(self, bg_color, fg_color, entry_bg_color):
        time_title = tk.Label(self.timezone_window, text="Nouveau Fuseau UTC : ", font=smallfont, bg=bg_color,
                              fg=fg_color)
        time_title.pack(pady=10)

        self.new_time_entry_var = tk.StringVar(value=str(self.time_offset))
        new_time_entry = tk.Entry(self.timezone_window, textvariable=self.new_time_entry_var, font=smallfont,
                                  bg=entry_bg_color, fg=fg_color, insertbackground=fg_color)
        new_time_entry.pack(pady=5)

        change_utc_btn = tk.Button(self.timezone_window, text="Changer l'UTC", command=self.update_timezone,
                                   bg=bg_color, fg=fg_color, activebackground=bg_color, activeforeground=fg_color)
        change_utc_btn.pack(pady=5)

    def create_time_adjustment_section(self, bg_color, fg_color, entry_bg_color):
        adjust_frame = tk.Frame(self.timezone_window, bg=bg_color)
        adjust_frame.pack(pady=20)

        hours_label = tk.Label(adjust_frame, text="Heures :", font=smallfont, bg=bg_color, fg=fg_color)
        hours_label.grid(row=0, column=0, padx=5)
        self.hours_entry_var = tk.StringVar(value="0")
        hours_entry = tk.Entry(adjust_frame, textvariable=self.hours_entry_var, font=smallfont, bg=entry_bg_color,
                               fg=fg_color, insertbackground=fg_color)
        hours_entry.grid(row=0, column=1, padx=5)

        minutes_label = tk.Label(adjust_frame, text="Minutes :", font=smallfont, bg=bg_color, fg=fg_color)
        minutes_label.grid(row=1, column=0, padx=5)
        self.minutes_entry_var = tk.StringVar(value="0")
        minutes_entry = tk.Entry(adjust_frame, textvariable=self.minutes_entry_var, font=smallfont,
                                 bg=entry_bg_color, fg=fg_color, insertbackground=fg_color)
        minutes_entry.grid(row=1, column=1, padx=5)

        seconds_label = tk.Label(adjust_frame, text="Secondes :", font=smallfont, bg=bg_color, fg=fg_color)
        seconds_label.grid(row=2, column=0, padx=5)
        self.seconds_entry_var = tk.StringVar(value="0")
        seconds_entry = tk.Entry(adjust_frame, textvariable=self.seconds_entry_var, font=smallfont,
                                 bg=entry_bg_color, fg=fg_color, insertbackground=fg_color)
        seconds_entry.grid(row=2, column=1, padx=5)

        adjust_btn = tk.Button(self.timezone_window, text="Ajuster l'heure", command=self.adjust_time, bg=bg_color,
                               fg=fg_color, activebackground=bg_color, activeforeground=fg_color)
        adjust_btn.pack(pady=5)
    ...
    def adjust_time(self):
        try:
            hours = int(self.hours_entry_var.get())
            minutes = int(self.minutes_entry_var.get())
            seconds = int(self.seconds_entry_var.get())

            current_time = self.get_datetime_with_utc()
            adjusted_time = current_time + datetime.timedelta(hours=hours, minutes=minutes, seconds=seconds)

            # Mettre à jour l'heure manuelle
            self.manual_time = adjusted_time

            messagebox.showinfo("Heure Ajustée", f"L'heure ajustée est : {adjusted_time.strftime('%Y-%m-%d %H:%M:%S')}")

            # Redémarrer les mises à jour périodiques des horloges
            self.restart_clock_updates()
        except ValueError:
            messagebox.showerror("Erreur", "Veuillez entrer des nombres entiers pour les heures, minutes et secondes.")

    def restart_clock_updates(self):
        # Redémarrer les mises à jour périodiques des horloges
        if hasattr(self, 'frames'):
            for frame in self.frames.values():
                if isinstance(frame, (Horloge, DHorloge)):
